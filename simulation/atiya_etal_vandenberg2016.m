...
dynamics_and_results_2nddecision = repmat(struct('block_no',0,'block_size',0,...
    ...
    'is_late_com',false,'motor_decision_made',false,'is_first_correct',false)...
    , n_of_trials*size(coherences,2), 1);



trial_type_string = 'reaction_time';
across_trial_string = 'across_trials_effect';
...
% coherences for loop
all_coherences=[0,3.2,6.4,12.8,25.6,51.2];
index = 0;
index2 =0;
...
for k = 1: no_of_blocks
    coh = coherences(k);
    trial_loop_size = n_of_trials*2;
    % Trials for loop.
    for i = 1:trial_loop_size
        
        if(~mod(i,2))
            coh = all_coherences(randi(numel(all_coherences)));
        end
        if(~mod(i,2))
            index2 = index2+1;
        else
        ...
        [y_1,y_2,s1,s2,y_mc_hu,y_mc_lu,decision_module_crossed,...
            response_time, energy] = nonlinear_decision_module_and_mc_integrate...
            (coh,decision_module_threshold, dynamics_and_results_2nddecision(index).c);
        ...
        if(~mod(i,2))
            dynamics_and_results_2nddecision(index2).block_no=k;
            dynamics_and_results_2nddecision(index2).block_size=n_of_trials;
            dynamics_and_results_2nddecision(index2).trial_no=i;
            dynamics_and_results_2nddecision(index2).coherence_level=coh;
            dynamics_and_results_2nddecision(index2).timestep_size=dt;
            dynamics_and_results_2nddecision(index2).trial_length=trial_length;
            dynamics_and_results_2nddecision(index2).stim_onset=stim_onset;
            dynamics_and_results_2nddecision(index2).stim_duration=stim_duration;
            dynamics_and_results_2nddecision(index2).y_1=y_1;
            dynamics_and_results_2nddecision(index2).y_2=y_2;
            dynamics_and_results_2nddecision(index2).y_5=y_5;
            dynamics_and_results_2nddecision(index2).y_6=y_6;
            dynamics_and_results_2nddecision(index2).y_mc_hu=y_mc_hu;
            dynamics_and_results_2nddecision(index2).y_mc_lu=y_mc_lu;
            dynamics_and_results_2nddecision(index2).decision_module_threshold = decision_module_threshold;
            dynamics_and_results_2nddecision(index2).motor_target_threshold = motor_target_threshold;
            dynamics_and_results_2nddecision(index2).motor_maximum_value_left = max(y_5);
            dynamics_and_results_2nddecision(index2).motor_maximum_value_right = max(y_6);
            dynamics_and_results_2nddecision(index2).decision_module_crossed = decision_module_crossed;
            dynamics_and_results_2nddecision(index2).motor_decision_made = motor_decision_made;
            dynamics_and_results_2nddecision(index2).is_motor_correct = is_motor_correct;
            dynamics_and_results_2nddecision(index2).movement_duration = movement_duration * dt;
            dynamics_and_results_2nddecision(index2).response_time = response_time * dt;
            dynamics_and_results_2nddecision(index2).is_motor_com = is_motor_com;
            dynamics_and_results_2nddecision(index2).is_late_com= is_late_com;
        else
            %%%% Save in struct (only second trial, whether first trial is correct + c value):
            if(index+1<=n_of_trials*no_of_blocks)
                dynamics_and_results_2nddecision(index2+1).c= calculate_control_value(energy);
                dynamics_and_results_2nddecision(index2+1).is_first_correct=is_motor_correct;
            end
        end
        %%%%%%%%%%%
        ...
        clearvars -except k i dt trial_length stim_onset...
            ...
            index index2 dynamics_and_results_2nddecision ...
            across_trial_effect all_coherences no_of_blocks trial_loop_size
        %%%%
    end
    
end
clearvars -except dynamics_and_results_2nddecision tStart filename
...
function [y_1,y_2,s1,s2,y_mc_uncertainty,y_mc_inhibitory,decision_module_crossed,response_time, energy] = nonlinear_decision_module_and_mc_integrate(coh,decision_module_threshold,control)
...
gain_mc_uncertainty = 1;                 % Gain of input-output function (cf. Wong & Wang, 2006)
tau_mc_uncertainty = 150;                % Membrane time constant (cf. Wilson and Cowan, 1972)
gain_mc_inhibitory = 1;                 % Gain of input-output function (cf. Wong & Wang, 2006)
tau_mc_inhibitory = 150;                % Membrane time constant (cf. Wilson and Cowan, 1972)

j_mc0 = 0.009;                  % (excitation) Coupling constant from uncertainty to eye module
j_mc_v_inh = 1;               % Coupling from decision module to uncertainty inhibitory population
mu= 30+control;                 % Constant input used for uncertainty I-O function.
inhibition_inhibitory = 0.5;    % Inhibition strength constant from inhibitory to uncertainty neural population
...
for t=1:trial_length-1
    ...
    i_hu_total = gain_mc_uncertainty * (mu - ...
        inhibition_inhibitory * y_mc_inhibitory(t)) - inh_top_down_inhibition - custom_top_down_inhibition;
    
    i_lu_total = gain_mc_inhibitory * (j_mc_v_inh * (y_1(t) +...
    ...
    y_mc_uncertainty(t+1) = y_mc_uncertainty(t) + (dt/tau_mc_uncertainty) * ...
        (-y_mc_uncertainty(t) +f_i_mc_hu);
    y_mc_inhibitory(t+1) = y_mc_inhibitory(t) + (dt/tau_mc_inhibitory) * ...
        (-y_mc_inhibitory(t) +f_i_mc_lu);
    
    overall_mc_input(t+1) = i_hu_total;
    %%%%%%%%%%%%%%%%%%%%%%%%%%%UNCERTAINTY MODULE INTEGRATE END
end

energy = trapz(y_mc_uncertainty);
return;

end % Function end

function [y_5,y_6] =  hand_module_integrate(y_7,y_8,response_time)
...
if(point_of_change-point_before_point_of_change < 100)
    return;
end
